...
int perturb_init(
 ...
 if (ppt->has_lvdm == _TRUE_) {
    ppt->G_cosmo_over_G_0=1./(1.+ppt->beta/2.+3./2.*ppt->lambda);
//    ppt->alpha=2.*ppt->beta;
    ppt->c_sq_chi  =(ppt->beta+ppt->lambda)/ppt->alpha;
    ppt->B         = (ppt->beta+3.*ppt->lambda)/ppt->alpha;
    ppt->H_alpha   = pba->H0/sqrt(ppt->alpha);
    ppt->k_y_0_sq =3.*pba->H0*pba->H0*ppt->Y_dm*pba->Omega0_cdm/(ppt->alpha*ppt->G_cosmo_over_G_0*(1.-ppt->Y_dm));
  }
...
//	if (pba->has_lvdm == _TRUE_)
//	  ppt->has_source_chidot = _TRUE_; //line for khronon
      }
...
//	if (pba->has_lvdm == _TRUE_)     //line for khronon
//	  ppt->has_source_chi = _TRUE_;	
      }
...
int perturb_vector_init(
  ...
  if _scalars_ {
    ...
    class_define_index(ppv->index_pt_chi,ppt->has_lvdm,index_pt,1); /*chi*/
    class_define_index(ppv->index_pt_chidot,ppt->has_lvdm,index_pt,1); /*chidot*/
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
	...
	ppv->y[ppv->index_pt_chi] =
	  ppw->pv->y[ppw->pv->index_pt_chi];
	
	ppv->y[ppv->index_pt_chidot] =
	  ppw->pv->y[ppw->pv->index_pt_chidot];
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if _scalars_ {
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
	 ...
	 theta_ur, shear_ur, tau */
        ...
        /* In LVDM the initial power spectrum is getting renormalised. In order to compare the PS with LCDM we
         define A_s_LVDM = A_s_lcdm at superhorizon scales, so we renormalise the initial curvature perturbation according to this as */
        
     ppr->curvature_ini = (ppt->G_cosmo_over_G_0 * (1.-ppt->beta)+2.*fracnu/15.)/(ppt->G_cosmo_over_G_0*(3./2.+3.*ppt->beta/4.+9.*ppt->lambda/4.)-1./2.)/(1.+2.*fracnu/15.);
	...
	* ppr->curvature_ini;
	...
	/* One then needs to modify also the IC for theta_nu, sigma_nu, eta, and setting om to 0 in the other formulas */
	/* Note from the paper, h0 = k2 * ppr->curvature_ini/ 2

	/* photon density */
	ppw->pv->y[ppw->pv->index_pt_delta_g] = - ktau_two/3. * ppr->curvature_ini; 
	
	/* photon velocity */
	ppw->pv->y[ppw->pv->index_pt_theta_g] = - k*ktau_three/36. * ppr->curvature_ini;
	...
	if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_)) {
	  ...
	  theta_ur = - k*ktau_three/18.*(23.+4.*fracnu-9.*ppt->beta+18.*ppt->lambda) / (30.+8.*fracnu-30.*ppt->beta) * ppr->curvature_ini; /* velocity of ultra-relativistic neutrinos/relics */
	  
	  shear_ur = ktau_two*(4.+3.*ppt->beta+9.*ppt->lambda)/(45.+12.*fracnu-45.*ppt->beta) * ppr->curvature_ini/2.; /* shear of ultra-relativistic neutrinos/relics */
	  
	  l3_ur = ktau_three*2./7./(12.*fracnu+45.)* ppr->curvature_ini;
	}
	
	/* synchronous metric perturbation eta */
	eta = ppr->curvature_ini / ppt->G_cosmo_over_G_0*(1.-ktau_two/24.*ppt->G_cosmo_over_G_0/(15.*ppt->G_cosmo_over_G_0*(1.-ppt->beta)+4.*fracnu)*(4.*fracnu+5.-45./2.*ppt->alpha*ppt->c_sq_chi*ppt->G_cosmo_over_G_0));
	
	ppw->pv->y[ppw->pv->index_pt_chi] = 2.*ppr->curvature_ini *ktau_two*tau*
	  (3.*ppt->c_sq_chi*ppt->alpha*(15.+4.*fracnu-15.*ppt->beta) + ppt->beta*(-8.*fracnu+5.*(-2.+9.*ppt->beta+9.*ppt->lambda)))/
	  (3.*(-49. + 1.-8.*ppt->B)*ppt->alpha*(15.+4.*fracnu-15.*ppt->beta));
	ppw->pv->y[ppw->pv->index_pt_chidot] = 6.*ppr->curvature_ini *ktau_two*
	  (3.*ppt->c_sq_chi*ppt->alpha*(15.+4.*fracnu-15.*ppt->beta) + ppt->beta*(-8.*fracnu+5.*(-2.+9.*ppt->beta+9.*ppt->lambda)))/
	  (3.*(-49. + 1.-8.*ppt->B)*ppt->alpha*(15.+4.*fracnu-15.*ppt->beta));
	/*ppw->pv->y[ppw->pv->index_pt_xi] = -ktau_two*tau*ppr->curvature_ini*c_sq_theta*one_over_tau_2/(4.*ppt->H_alpha);*/
	/*ppw->pv->y[ppw->pv->index_pt_xidot] = -ktau_two*ppr->curvature_init*c_sq_theta*one_over_tau_2/(4.*ppt->H_alpha);*/
//	ppw->pv->y[ppw->pv->index_pt_xi] = 0.;
//	ppw->pv->y[ppw->pv->index_pt_xidot] = 0.;
      }
	...
	/* alpha is upsilon in TCDM paper*/
	alpha = ppr->curvature_ini*0.5*tau*(1.-(5.+4.*fracnu-45./2.*ppt->alpha*ppt->c_sq_chi*ppt->G_cosmo_over_G_0)/(15.*ppt->G_cosmo_over_G_0*(1.-ppt->beta)+4.*fracnu) );

	ppw->pv->y[ppw->pv->index_pt_chi] += alpha;
	// Is alpha derived here also ?
	ppw->pv->y[ppw->pv->index_pt_chidot] += alpha/tau;

	// need background call here to get a function of tau.
	/*class_call(background_at_tau(pba,*/
				     /*tau, */
				     /*pba->normal_info, */
				     /*pba->inter_closeby, */
	...
	//Define temporary variable theta bar dot IN THE CASE OF s_c = -1, then s_theta = -1
	/*theta_bar_dot = -1.*a*sqrt(3./(4.*_pi_*ppt->G_cosmo_over_0));*/
	/*one_over_tau_1     = ppt->s_theta*a*sqrt(6.*rho_nde*ppt->c_sq_theta/ppt->alpha);*/
//	ppw->pv->y[ppw->pv->index_pt_xi] += pba->Omega0_lambda*sqrt(ppt->c_sq_theta)*tau*tau*sqrt(0.5)*pba->H0;
//	ppw->pv->y[ppw->pv->index_pt_xidot] += sqrt(0.5)*pba->Omega0_lambda*sqrt(ppt->c_sq_theta)*tau*pba->H0;
...
int perturb_einstein(
 ...
 /* khronon contribution: lvdm */
    /* following CLASS units, 
       - [delta rho] is in fact [(8piG_cosmo/3) delta rho]
         if [delta rho = M0^2 * something], then
         [(8piG_cos/3) delta rho] is [(8piGM0^2/3) delta rho] = [1/3 (G_cosmo/G_0) delta rho]
    */

    if (ppt->has_lvdm == _TRUE_) {
       
      delta_rho += ppt->G_cosmo_over_G_0/3. *k2/a2 * (a_prime_over_a*(ppt->alpha-ppt->beta-3.*ppt->lambda)*y[ppw->pv->index_pt_chi]+ppt->alpha*y[ppw->pv->index_pt_chidot]);

      rho_plus_p_theta += ppt->G_cosmo_over_G_0/3./a2*((ppt->beta+ppt->lambda)*k2*k2*y[ppw->pv->index_pt_chi]);

      delta_p +=  ppt->G_cosmo_over_G_0 * (ppt->beta+3.*ppt->lambda)/9.*k2/a2*(y[ppw->pv->index_pt_chidot]+2.*a_prime_over_a*y[ppw->pv->index_pt_chi]);

      rho_plus_p_shear -=  2./9.*k2/a2*ppt->G_cosmo_over_G_0 * ppt->beta*(y[ppw->pv->index_pt_chidot]+2*a_prime_over_a*y[ppw->pv->index_pt_chi]);
	...
	/* Modification of matter power spectrum definition */
	ppw->delta_pk = 1./ppt->G_cosmo_over_G_0*(delta_rho - (1.-ppt->G_cosmo_over_G_0)/3./a/a*a_prime_over_a*ppw->pvecmetric[ppw->index_mt_h_prime])/rho_pk;
      }
   ...
   if (ppt->has_lvdm == _TRUE_) {
	ppw->pvecmetric[ppw->index_mt_h_prime] = 
	  ( 2.*k2 * y[ppw->pv->index_pt_eta] + 3. * a2 * delta_rho/ppt->G_cosmo_over_G_0)*ppt->G_cosmo_over_G_0/a_prime_over_a;
      }
      else {
	ppw->pvecmetric[ppw->index_mt_h_prime] = 
	  ( (k2-K) * y[ppw->pv->index_pt_eta] + 1.5 * a2 * delta_rho)
...
if (ppt->has_lvdm == _TRUE_) {
	ppw->pvecmetric[ppw->index_mt_eta_prime] = ((ppt->beta+ppt->lambda)/2.*k2*ppw->pvecmetric[ppw->index_mt_h_prime]+
					    3.*a2/ppt->G_cosmo_over_G_0*rho_plus_p_theta)/2./k2/(1.-ppt->beta);  /* eta' */
      }
      else {
	...
	ppw->pvecmetric[ppw->index_mt_h_prime_prime] = 
	  ...
	  +2.*k2*ppt->G_cosmo_over_G_0*y[ppw->pv->index_pt_eta]
	  -9.*a2*delta_p;
      }
      else {
      ppw->pvecmetric[ppw->index_mt_h_prime_prime] = 
   ...
   if (ppt->has_lvdm == _TRUE_) {
	ppw->pvecmetric[ppw->index_mt_alpha_prime] = 
	  (- 4.5 * (a2/k2) /ppt->G_cosmo_over_G_0 * rho_plus_p_shear + y[ppw->pv->index_pt_eta])/(1.-ppt->beta) - 2.*a_prime_over_a*alpha; /* alpha' = (h''+6eta'')/2k2 */
      }
      else {
 ppw->pvecmetric[ppw->index_mt_alpha_prime] = 
    ...
    /*class_stop(ppt->error_message,"%e %e %e %e\n",ppw->pvecmetric[ppw->index_mt_alpha_prime],ppw->pvecmetric[ppw->index_mt_h_prime],shear_g, ppw->pvecback[pba->index_bg_rho_g]);*/
  }
...
//              + pvecthermo[pth->index_th_g] / 4. * (delta_g + Pi / 4.);
            ...
            /*Sachs-Wolfe*/
//            source_term_table[index_type][index_tau * ppw->st_size + ppw->index_st_S0] =
//              pvecthermo[pth->index_th_g] / 4. * (delta_g + 0.*Pi / 4.)
//              + pvecthermo[pth->index_th_g] * pvecmetric[ppw->index_mt_alpha_prime];
...
//              + 3./16. * pvecthermo[pth->index_th_g] * Pi_prime / k2;
            ...
            /*Sachs-Wolfe*/
//            source_term_table[index_type][index_tau * ppw->st_size + ppw->index_st_dS2] =
//              0.*3./16. * pvecthermo[pth->index_th_dg] * Pi / k2
//              + 0.*3./16. * pvecthermo[pth->index_th_g] * Pi_prime / k2;
...
//  if((k>0.1)&&(k<0.102)){
  ...
  //  if ((k>=8e-05) && (k<0.0001)) {
    if (k<7.2e-6) {
//    if (k<0.0001) {
//    if ((k>=9.76063e-03) && (k<9.76064e-03)) {
// large if ((k>=9.561170e-05) && (k<9.561185e-05)) {
// smaller if ((k>=4.80e-02) && (k<4.90e-02)) {
//if ((k>=4.80e-02) && (k<4.90e-02)) {
//if ((k>=4.75e-01) && (k<4.9e-01)) {
	...
	shear_ur = 0.;
     ...
     /* gravitational potential from Ma and Bertschinger eq. 15*/
    if (ppt->gauge == synchronous) {
    ...
    double delta_b,delta_cdm;
    double delta_rho_chi;

    /*double chi_dot;*/
    /*chi_dot = ppt->G_cosmo_over_G_0/3. *k2/a2 * (ppt->alpha*y[ppw->pv->index_pt_chidot]);*/


    delta_b = y[ppw->pv->index_pt_delta_b];
    delta_cdm = y[ppw->pv->index_pt_delta_cdm];
    
    double delta_rho_b,delta_rho_cdm;

    /* Gauge transformation to newtonian for b, cdm and g */
    delta_g += (-4.*a_prime_over_a)*(pvecmetric[ppw->index_mt_h_prime] + 6. * pvecmetric[ppw->index_mt_eta_prime])/2./k2 ;
    delta_b += -3.*a_prime_over_a*(pvecmetric[ppw->index_mt_h_prime] + 6. * pvecmetric[ppw->index_mt_eta_prime])/2./k2 ;
    delta_cdm += -3.*a_prime_over_a*(pvecmetric[ppw->index_mt_h_prime] + 6. * pvecmetric[ppw->index_mt_eta_prime])/2./k2 ;

    delta_rho_b   = delta_b*ppw->pvecback[pba->index_bg_rho_b]; 
    delta_rho_cdm = delta_cdm*ppw->pvecback[pba->index_bg_rho_cdm]; 
    /*Same transformation ??? for the delta_rhos*/      
    /*delta_rho_chi += (2.*a_primeprime_over_a/a_prime_over_a - 4.*a_prime_over_a)*(pvecmetric[ppw->index_mt_h_prime] + 6. * pvecmetric[ppw->index_mt_eta_prime])/2./k2 ;*/
 

    double chi,chi_dot;
   
    chi = y[ppw->pv->index_pt_chi];
    chi_dot = y[ppw->pv->index_pt_chidot];


    /*Gauge transform for chi */
    chi += (pvecmetric[ppw->index_mt_h_prime] + 6. * pvecmetric[ppw->index_mt_eta_prime])/2./k2 ;
    chi_dot += pvecmetric[ppw->index_mt_alpha_prime];


    /* Computation of delta_rhos, now in newtonian (eq 46 Draft_March13.pdf)*/
    delta_rho_chi = ppt->G_cosmo_over_G_0/3. *k2/a2 * (a_prime_over_a*(ppt->alpha-ppt->beta-3.*ppt->lambda)*chi+ppt->alpha*chi_dot);
...
//    if (pba->has_cdm == _TRUE_)
//       fprintf(stdout,"%e   ",y[ppw->pv->index_pt_delta_cdm]);

    if (ppt->has_lvdm == _TRUE_) {

	fprintf(stdout,"%e  %e ",
	  /*  y[ppw->pv->index_pt_chi],*/
	  /*  y[ppw->pv->index_pt_chidot]);*/
            chi, 
            delta_rho_chi);
    }
...
int perturb_derivs(double tau,
  ...
  if _scalars_ {
    ...
    if (pba->has_cdm == _TRUE_) {  
      ...
      /** ---> newtonian gauge: cdm density and velocity. Equations should ne changed!!! */
	...
	dy[ppw->pv->index_pt_theta_cdm] = - a_prime_over_a*y[ppw->pv->index_pt_theta_cdm] + metric_euler; /* cdm velocity */
      ...
      if (ppt->gauge == synchronous) {
	dy[ppw->pv->index_pt_delta_cdm] =ppt->Y_dm/(1.-ppt->Y_dm)*k2*y[ppw->pv->index_pt_chi] -metric_continuity; /* cdm density */
      }
    ...
    if (ppt->gauge == synchronous) {
      ...
      dy[ppw->pv->index_pt_chi] = y[ppw->pv->index_pt_chidot];


      double calH_prime = a_primeprime_over_a-a_prime_over_a*a_prime_over_a;

      dy[ppw->pv->index_pt_chidot] = 
	-k2*ppt->c_sq_chi*y[ppw->pv->index_pt_chi]
	-2.*a_prime_over_a*y[ppw->pv->index_pt_chidot]
	-((1.+ppt->B)*a_prime_over_a*a_prime_over_a+(1.-ppt->B)*calH_prime+ppt->k_y_0_sq/ppw->pvecback[pba->index_bg_a])*y[ppw->pv->index_pt_chi]
	-ppt->c_sq_chi/2.*pvecmetric[ppw->index_mt_h_prime]
	-2.*ppt->beta/ppt->alpha*pvecmetric[ppw->index_mt_eta_prime];
